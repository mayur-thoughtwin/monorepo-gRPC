...
export class WelcomeTemplate extends BaseTemplate<WelcomeTemplateData> {
  getSubject(data: WelcomeTemplateData): string {
    return `🎉 Welcome to MonoRepo gRPC App, ${data.name}!`;
  }

  getHtml(data: WelcomeTemplateData): string {
  ...
  getText(data: WelcomeTemplateData): string {
    return `
Welcome, ${data.name}!
